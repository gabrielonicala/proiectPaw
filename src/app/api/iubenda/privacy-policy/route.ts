...
export async function GET() {
  try {
    // Fetch the privacy policy from iubenda's no-markup endpoint
    const response = await fetch('https://www.iubenda.com/privacy-policy/70554621/no-markup', {
      headers: {
    ...
    // Remove everything from start until "Table of" or "Summary"
    const tableOfContentsIndex = html.search(/Table of|Summary/i);
